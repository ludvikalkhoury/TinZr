...
#include "TinZrCore.h"
#include "TinZrConnect.h"   // for _net sends
#include <esp_wifi.h>       // esp_wifi_set_storage, esp_wifi_restore
#include <WiFi.h>
#include <string.h>

// Helper: keep Wi-Fi state in RAM only (no driver NVS by default)
...
void TinZrConsole::begin(const TinZrConsoleDefaults& def, uint32_t connect_timeout_ms) {
	...
	// For use_static: always take from DEF.
...
#if TINZR_ENABLE_OTA
	// When OTA is compiled in, let TinZrOTA run its state machine
	_ota.handle();
#endif
}
...
void TinZrConsole::applyConfig(uint32_t connect_timeout_ms) {
	...
	// ---------------------------
	// Case 1: OTA is compiled in → use TinZrOTA to manage Wi-Fi + OTA + LEDs
	// ---------------------------
#if TINZR_ENABLE_OTA
	// Re-init TinZrOTA so LED/connection state machine restarts
	_ota.~TinZrOTA();
	new (&_ota) TinZrOTA();
	...
	// You can also fill cfg.ip/gw/mask/dns1/dns2 and led_brightness if desired

	_ota.begin(_host.c_str(), cfg, connect_timeout_ms);
	return;
#else
	// ---------------------------
	// Case 2: OTA not compiled → Wi-Fi-only, but with "OTA-like" init pattern
	// ---------------------------
	Serial.println("🌐 Wi-Fi connect (no OTA)");

	// Very similar init to TinZrOTA::connectWiFi(), minus ArduinoOTA and LED updates
	WiFi.mode(WIFI_STA);
	WiFi.setSleep(false);
	WiFi.setTxPower(WIFI_POWER_8_5dBm);   // default tx power you like
	WiFi.persistent(false);
	WiFi.disconnect(true, true);
	delay(300);
	...
	if (_use_static) {
		...
		// If you later store IP/gw/mask/dns in NVS, call WiFi.config() here.
		// For now we keep DHCP unless you wire in real static fields.
	} else {
	...
	if (WiFi.status() == WL_CONNECTED) {
		Serial.println("✅ Wi-Fi connected (no OTA).");
		Serial.print("📡 IP: ");
		Serial.println(WiFi.localIP());
	} else {
		Serial.println("❌ Wi-Fi connect timeout (no OTA).");
	}
#endif
}
...
#if TINZR_ENABLE_OTA
	bool connected = _ota.connected();
	Serial.printf("Connected : %s\n", connected ? "YES" : "NO");
	if (connected) {
		IPAddress ip = _ota.ip();
		Serial.printf("IP        : %s\n", ip.toString().c_str());
		Serial.printf("mDNS/Host : %s.local\n", _host.c_str());
	}
#else
	// OTA not compiled at all → always use Wi-Fi status
...
void TinZrConsole::handleSerial() {
	...
	if (cmd.startsWith("WIFI ")) {
		...
		if (_autosave_wifi) {
			saveToNVS();
			Serial.println("🔄 Rebooting to apply and restart Wi-Fi/OTA…");
			delay(200);
			ESP.restart();
			return; // not reached
		} else {
	...
	if (cmd.equalsIgnoreCase("WIPEWIFI")) {
		wipeWiFiDriverNVS();
		return;
	}
